...
namespace AIChatBot.API.Controllers
{
    ...
    public class FilesController : ControllerBase
    {
        ...
        public async Task<IActionResult> DownloadFile(int fileId, [FromQuery] Guid userId)
        {
            try
            {
                if (userId == Guid.Empty)
                ...
                var agentFile = await _fileService.GetFileAsync(fileId, userId);
                if (agentFile == null)
                {
                    return NotFound("File not found or access denied");
                }

                var fileStream = await _fileService.GetFileStreamAsync(fileId, userId);
                if (fileStream == null)
                {
